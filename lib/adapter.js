...
async function textAndPdfWireParts(text, resolvePdf, pdfBudget) {
	const parts = [];
	let offset = 0;
	...
	for (let match = marker.exec(text); match !== null; match = marker.exec(text)) {
		if (match.index > offset) parts.push({ type: "input_text", text: text.slice(offset, match.index) });
		...
		const pdf = await resolvePdf(match[1]);
...
async function userContentItems(blocks, requestImages, representation, resolveFileId, resolvePdf, pdfBudget, messageIndex, nextImage) {
	const parts = [];
	for (const block of blocks) {
		if (block.type === "text") {
			if (block.text.length > 0) parts.push(...await textAndPdfWireParts(block.text, resolvePdf, pdfBudget));
			continue;
		}
		if (block.type === "image") {
			nextImage.value += 1;
...
async function buildInputItems(options, representation, requestImages, resolveFileId, resolvePdf, maxPdfRequestBytes = DEFAULT_MAX_PDF_BYTES) {
	const items = [];
	...
	for (const [messageIndex, message] of options.messages.entries()) {
		...
		const content = await userContentItems(regular, requestImages, representation, resolveFileId, resolvePdf, pdfBudget, messageIndex + 1, nextImage);
		if (content.length > 0 || toolResults.length === 0) {
			flushToolImages();
			items.push({ role: "user", content });
		}
		for (const result of toolResults) {
			const parts = await userContentItems(result.content, requestImages, representation, resolveFileId, resolvePdf, pdfBudget, messageIndex + 1, nextImage);
...
export var VeArkAdapter = class extends LlmAdapter {
	...
	async *streamWithConnection(options, connection) {
		...
		try {
			const iterator = this.request(options, watchdog.signal, connection, apiKey, attachments, resolvePdf, () => {
				watchdog.pulse();
	...
	async *request(options, signal, connection, apiKey, attachments, resolvePdf, onActivity) {
		...
		while (true) {
			...
			try {
				...
				input = await buildInputItems(serializeOptions, representation, requestImages, hasImages && attachments !== void 0 ? resolveFileId : void 0, resolvePdf, connection.maxPdfRequestBytes);
			} catch (error) {
