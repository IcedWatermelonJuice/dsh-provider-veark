...
import { DEFAULT_MAX_PDF_BYTES, PDF_MARKER_RE } from "./pdf-store.js";
...
/** Ark 64 MiB 整体请求上限下，为 SDK/HTTP 封装保留 1 MiB。 */
export const DEFAULT_MAX_REQUEST_BODY_BYTES = 63 * 1024 * 1024;
/** raw 字节移除步长。 */
...
function contentHasPdfMarker(content) {
	if (typeof content === "string") return new RegExp(PDF_MARKER_RE.source, PDF_MARKER_RE.flags).test(content);
	return Array.isArray(content) && content.some((block) => block.type === "text" && new RegExp(PDF_MARKER_RE.source, PDF_MARKER_RE.flags).test(block.text));
}

const AT_PDF_STOP = /[\s\u0000-\u001f\u007f-\u009f"'`<>|{},;:!?，。；：！？、]/u;

/** 扫描普通用户文本中的 @PDF；返回原文区间，保证失败时可以逐字放行。 */
export function scanAtPdfReferences(text) {
	const found = [];
	for (let start = text.indexOf("@"); start >= 0; start = text.indexOf("@", start + 1)) {
		const before = text[start - 1];
		if (before !== void 0 && /[A-Za-z0-9._%+-]/u.test(before)) continue;
		if (text[start + 1] === '"') {
			let value = "";
			let cursor = start + 2;
			let closed = false;
			for (; cursor < text.length; cursor++) {
				...
				if (char === '"') { closed = true; cursor++; break; }
				if (char === "\\" && (text[cursor + 1] === "\\" || text[cursor + 1] === '"')) value += text[++cursor];
				else value += char;
			}
			if (closed && value.toLowerCase().endsWith(".pdf") && !/[\u0000-\u001f\u007f-\u009f]/u.test(value)) found.push({ start, end: cursor, path: value, raw: text.slice(start, cursor) });
			continue;
		}
		let cursor = start + 1;
		while (cursor < text.length && !AT_PDF_STOP.test(text[cursor])) cursor++;
		let candidate = text.slice(start + 1, cursor);
		while (/[.)\]]$/u.test(candidate) && !candidate.toLowerCase().endsWith(".pdf")) candidate = candidate.slice(0, -1);
		if (!candidate.toLowerCase().endsWith(".pdf")) continue;
		const end = start + 1 + candidate.length;
		found.push({ start, end, path: candidate, raw: text.slice(start, end) });
	}
	return found;
}

function virtualPdfToken(path) {
	const match = /^\.dsh-pdf\/([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\/([^/]+\.pdf)$/iu.exec(path.replaceAll("\\", "/"));
	if (match === null) return void 0;
	try {
		const name = decodeURIComponent(match[2]);
		if (/[\u0000-\u001f\u007f-\u009f/\\]/u.test(name) || !name.toLowerCase().endsWith(".pdf")) return void 0;
...
function addPdfPart(parts, pdf, pdfBudget) {
	pdfBudget.value += pdf.data.length;
	if (pdfBudget.value > pdfBudget.max) throw new LlmError(`Ark inline PDF input exceeds the ${pdfBudget.max}-byte safe request budget. Start a new session or attach a smaller document.`, "INVALID_REQUEST");
	parts.push({
		type: "input_file",
		filename: pdf.name,
		file_data: `data:${pdf.mediaType};base64,${Buffer.from(pdf.data).toString("base64")}`
	});
}

/** Responses API 的 user 内容始终用数组形式（pi-ai 现网形状）。 */
async function textAndPdfWireParts(text, resolvePdf, resolveAtPdf, allowAtPdf, pdfBudget, scanAt = true) {
	const parts = [];
	let offset = 0;
	const marker = new RegExp(PDF_MARKER_RE.source, PDF_MARKER_RE.flags);
	const matches = [];
	for (let match = marker.exec(text); match !== null; match = marker.exec(text)) matches.push({ start: match.index, end: match.index + match[0].length, kind: "marker", token: match[1], raw: match[0] });
	if (scanAt && allowAtPdf) for (const ref of scanAtPdfReferences(text)) matches.push({ ...ref, kind: "at" });
	matches.sort((a, b) => a.start - b.start || (a.kind === "marker" ? -1 : 1));
	for (const match of matches) {
		if (match.start < offset) continue;
		if (match.start > offset) parts.push({ type: "input_text", text: text.slice(offset, match.start) });
		if (match.kind === "marker") {
			if (resolvePdf === void 0) throw new LlmError("This PDF attachment is unavailable outside its Volcengine provider session.", "UNSUPPORTED_CONTENT");
			addPdfPart(parts, await resolvePdf(match.token), pdfBudget);
		} else if (!allowAtPdf || resolveAtPdf === void 0) parts.push({ type: "input_text", text: match.raw });
		else {
			const virtual = virtualPdfToken(match.path);
			let pdf;
			if (virtual !== void 0) {
				if (resolvePdf === void 0) throw new LlmError("This PDF attachment is unavailable outside its Volcengine provider session.", "UNSUPPORTED_CONTENT");
				pdf = await resolvePdf(virtual.token);
			} else if (/^\.dsh-pdf[\\/]/iu.test(match.path)) pdf = void 0;
			else pdf = await resolveAtPdf(match.path);
			if (pdf === void 0) parts.push({ type: "input_text", text: match.raw });
			else addPdfPart(parts, pdf, pdfBudget);
		}
		offset = match.end;
	}
	if (offset < text.length) parts.push({ type: "input_text", text: text.slice(offset) });
	return parts;
}

async function userContentItems(blocks, requestImages, representation, resolveFileId, resolvePdf, resolveAtPdf, allowAtPdf, pdfBudget, messageIndex, nextImage, scanAt = true) {
	const parts = [];
	for (const block of blocks) {
		if (block.type === "text") {
			if (block.text.length > 0) parts.push(...await textAndPdfWireParts(block.text, resolvePdf, resolveAtPdf, allowAtPdf, pdfBudget, scanAt));
			continue;
		}
		if (block.type === "image") {
			nextImage.value += 1;
...
async function buildInputItems(options, representation, requestImages, resolveFileId, resolvePdf, resolveAtPdf, allowAtPdf, maxPdfRequestBytes = DEFAULT_MAX_PDF_BYTES) {
	const items = [];
	const pdfBudget = { value: 0, max: maxPdfRequestBytes };
	...
	for (const [messageIndex, message] of options.messages.entries()) {
		...
		const content = await userContentItems(regular, requestImages, representation, resolveFileId, resolvePdf, resolveAtPdf, allowAtPdf, pdfBudget, messageIndex + 1, nextImage);
		if (content.length > 0 || toolResults.length === 0) {
			flushToolImages();
			items.push({ role: "user", content });
		}
		for (const result of toolResults) {
			const parts = await userContentItems(result.content, requestImages, representation, resolveFileId, resolvePdf, resolveAtPdf, allowAtPdf, pdfBudget, messageIndex + 1, nextImage, false);
...
export var VeArkAdapter = class extends LlmAdapter {
	...
	async *streamWithConnection(options, connection) {
		...
		const hasPdfs = options.messages.some((message) => contentHasPdfMarker(message.content));
		const configuredModel = connection.models.find((entry) => entry.id === options.model);
		const allowAtPdf = configuredModel?.inputModalities?.includes("pdf") === true;
		let attachments;
		if (hasImages) {
			if (configuredModel?.inputModalities?.includes("image") !== true) throw new LlmError(`Ark model "${options.model}" does not accept image input.`, "UNSUPPORTED_CONTENT");
		...
		if (hasPdfs && configuredModel?.inputModalities?.includes("pdf") !== true) throw new LlmError(`Ark model "${options.model}" does not accept PDF input. Enable the "pdf" capability for this model only if its endpoint supports document understanding.`, "UNSUPPORTED_CONTENT");
		const apiKey = await this.config.resolveApiKey(connection);
		const pdfCache = /* @__PURE__ */ new Map();
		const resolvePdf = this.config.resolvePdf === void 0 ? void 0 : async (token) => {
			if (options.sessionId === void 0) throw new LlmError("Ark PDF attachments require a session id.", "INVALID_REQUEST");
			let pending = pdfCache.get(token);
			if (pending === void 0) {
				pending = this.config.resolvePdf(options.sessionId, token);
				pdfCache.set(token, pending);
			}
			try {
				return await pending;
			} catch (error) {
				throw new LlmError(`Unable to resolve Ark PDF attachment ${token}.`, "UNSUPPORTED_CONTENT", { cause: error });
			}
		};
		const workspacePdfCache = /* @__PURE__ */ new Map();
		const resolveAtPdf = this.config.resolveWorkspacePdf === void 0 || options.sessionId === void 0 ? void 0 : async (path) => {
			let pending = workspacePdfCache.get(path);
			if (pending === void 0) {
				pending = Promise.resolve(this.config.resolveWorkspacePdf(options.sessionId, path)).catch(() => void 0);
				workspacePdfCache.set(path, pending);
			}
			return pending;
		};
		...
		try {
			const iterator = this.request(options, watchdog.signal, connection, apiKey, attachments, resolvePdf, resolveAtPdf, allowAtPdf, () => {
				watchdog.pulse();
	...
	async *request(options, signal, connection, apiKey, attachments, resolvePdf, resolveAtPdf, allowAtPdf, onActivity) {
		...
		while (true) {
			...
			try {
				...
				input = await buildInputItems(serializeOptions, representation, requestImages, hasImages && attachments !== void 0 ? resolveFileId : void 0, resolvePdf, resolveAtPdf, allowAtPdf, connection.maxPdfRequestBytes);
			} catch (error) {
			...
			if (requestBodyBytes > (connection.maxRequestBodyBytes ?? DEFAULT_MAX_REQUEST_BODY_BYTES)) throw new LlmError(`Ark request body exceeds the ${(connection.maxRequestBodyBytes ?? DEFAULT_MAX_REQUEST_BODY_BYTES)}-byte safe limit. Remove large inline PDFs, images, or history.`, "INVALID_REQUEST");
