window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var PDF_MARKER_PREFIX = "[[dsh-provider-veark:pdf:";
	...
	function modelInvalid(model) {
		...
		var zh = {
			...
			pdfClaimHint: "输入关于所选 PDF 的问题",
			pdfProviderOnly: "PDF 输入仅适用于火山方舟 provider。",
			pdfModelUnsupported: "当前火山方舟模型未声明 PDF 能力。请仅在对应 endpoint 确实支持文档理解时，在模型列表中启用 pdf。",
			pdfSelectFirst: "请先通过 PDF 按钮选择一个文档。",
			pdfUploadFailed: "PDF 暂存失败。",
			pdfSessionUnavailable: "当前会话不可用。",
			pdfDefaultQuestion: "阅读此 PDF，并将其作为当前编码任务的上下文。",
			pdfAttachment: "PDF 附件",
			pdfPromptRejected: "PDF 问题未被会话接受。",
			pdfCandidateName: "PDF · 火山方舟文档理解",
			pdfCandidateDescription: "给下一次 provider 请求附加 PDF",
			pdfSelected: "已选择 PDF",
		...
		var en = {
			...
			pdfClaimHint: "Ask a question about the selected PDF",
			pdfProviderOnly: "PDF input is available only for the Volcengine provider.",
			pdfModelUnsupported: "The selected Volcengine model does not declare PDF support. Enable pdf in the model list only when that endpoint supports document understanding.",
			pdfSelectFirst: "Select a PDF with the PDF button first.",
			pdfUploadFailed: "PDF staging failed.",
			pdfSessionUnavailable: "The current session is unavailable.",
			pdfDefaultQuestion: "Read this PDF and use it as context for the current coding task.",
			pdfAttachment: "PDF attachment",
			pdfPromptRejected: "The PDF prompt was rejected.",
			pdfCandidateName: "PDF · Volcengine document",
			pdfCandidateDescription: "Attach a PDF to the next provider request",
		...
		var PdfClientController = class {
			constructor(ctx, sessions, models, settings, t) {
				this.ctx = ctx;
			...
			async providerFor(sessionId) {
				var current = await this.currentModel(sessionId);
			...
			async choose(sessionId, draft, inputActions) {
				...
				picker.onchange = async () => {
					...
					try {
						...
						this.pending.set(sessionId, { name: file.name, bytes: bytes.length, data: bytesToBase64(bytes) });
						store.set({ name: file.name, bytes: bytes.length, busy: false, error: "" });
						var text = typeof draft === "string" ? draft.trim() : "";
						inputActions.setDraft("/pdf " + (text.startsWith("/pdf") ? text.slice(4).trimStart() : text));
					} catch (error) {
			...
			claim(sessionId) {
				return {
					...
					submit: async (args, actx) => {
						if (await this.providerFor(sessionId) !== "volcengine") return { kind: "error", text: this.t("pdfProviderOnly") };
						if (!await this.supportsPdf(sessionId)) return { kind: "error", text: this.t("pdfModelUnsupported") };
						var pending = this.pending.get(sessionId);
						if (pending === void 0) return { kind: "error", text: this.t("pdfSelectFirst") };
						var staged = await this.ctx.remote.vearkPdf.stage({ sessionId: sessionId, name: pending.name, mediaType: "application/pdf", data: pending.data });
						if (!staged.ok) return { kind: "error", text: staged.error.message || this.t("pdfUploadFailed") };
						var session = this.sessions.sessionOf(actx);
						if (session === void 0) return { kind: "error", text: this.t("pdfSessionUnavailable") };
						var question = args.trim() || this.t("pdfDefaultQuestion");
						var marker = PDF_MARKER_PREFIX + staged.value.token + "]]";
						var accepted = await session.prompt([{ type: "text", text: this.t("pdfAttachment") + " (" + staged.value.name + "): " + marker + "\n" + question }], "queue");
						if (!accepted.ok) return { kind: "error", text: accepted.error.message || this.t("pdfPromptRejected") };
						this.pending.delete(sessionId);
						this.storeFor(sessionId).set({ name: "", bytes: 0, busy: false, error: "" });
						return { kind: "success" };
					}
				};
			}
			source() {
				return {
					...
					candidates: async (session, req) => await this.supportsPdf(session.sessionId) && "pdf".startsWith(req.query.toLowerCase()) ? [{ name: this.t("pdfCandidateName"), description: this.t("pdfCandidateDescription"), value: "pdf" }] : [],
					onPick: (pick) => pick.candidate.value === "pdf" ? { claim: this.claim(pick.session.sessionId) } : void 0,
					matchEnter: async (session, line) => /^\/pdf(?:\s|$)/iu.test(line) && await this.providerFor(session.sessionId) === "volcengine" ? { claim: this.claim(session.sessionId) } : void 0
				};
			}
		};
		function PdfAttachButton(props) {
			...
			return h("button", {
				type: "button",
				disabled: disabled,
				title: pdf.error || (pdf.name ? props.t("pdfSelected") + ": " + pdf.name : props.t("pdfAttachTitle")),
				"aria-label": props.t("pdfAttachAria"),
				onClick: function () { props.choosePdf(props.input.draft, props.inputActions); },
				style: { appearance: "none", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 7, background: "transparent", color: pdf.error ? "var(--dsw-alias-label-error)" : "var(--dsw-alias-label-secondary)", cursor: disabled ? "default" : "pointer", padding: "3px 8px", fontSize: 12, opacity: disabled ? .5 : 1 }
			}, pdf.busy ? "PDF…" : pdf.name ? "PDF ✓" : "PDF");
		}
		async function apply(ctx) {
			...
			ctx.inject(["remote.vearkPdf"], function (pdfCtx) {
				var sessions = pdfCtx.get("sessions");
				var inputTriggers = pdfCtx.get("inputTriggers");
				var modelDirectories = pdfCtx.get("modelDirectories");
				if (sessions === void 0 || inputTriggers === void 0 || modelDirectories === void 0) return;
				var pdfController = new PdfClientController(pdfCtx, sessions, modelDirectories, scope, t);
