window.__ModuleLoader__.load({
	...
	function modelInvalid(model) {
		...
		var PdfClientController = class {
			constructor(ctx, sessions, models, t) {
				this.ctx = ctx;
			...
			async providerFor(sessionId) {
				...
				return directory.store.getSnapshot().current && directory.store.getSnapshot().current.provider;
			}
			...
			source() {
				return {
					...
					candidates: async (session, req) => await this.providerFor(session.sessionId) === "volcengine" && "pdf".startsWith(req.query.toLowerCase()) ? [{ name: this.t("pdfCandidateName"), description: this.t("pdfCandidateDescription"), value: "pdf" }] : [],
		...
		function PdfAttachButton(props) {
			var provider = props.useVearkModel(function (state) { return state.current && state.current.provider; });
		...
		async function apply(ctx) {
			...
			ctx.inject(["remote.vearkPdf"], function (pdfCtx) {
				...
				var pdfController = new PdfClientController(pdfCtx, sessions, modelDirectories, t);
				...
				pdfCtx.slots.inject("conversation.input.left", function* () {
					yield pdfCtx.slots.register({
						...
						inject: function (sessionId) {
							return {
								hooks: { vearkPdf: pdfController.storeFor(sessionId), vearkModel: modelDirectories.directoryFor(sessionId).store },
