...
import { DEFAULT_MAX_PDF_BYTES, DEFAULT_PDF_RETENTION_DAYS, PDF_MEDIA_TYPE, VeArkPdfService, VeArkPdfStore, validatePdfBytes } from "./pdf-store.js";
...
/** 只读解析会话 cwd；内部存储不可用时安全降级，不使用进程 cwd。 */
export async function resolveWorkspacePdf(sessionId, relativePath, storagePath = dshHomePath("storages", "session_projcache.json"), maxBytes = DEFAULT_MAX_PDF_BYTES) {
	if (typeof sessionId !== "string" || sessionId.length === 0 || typeof relativePath !== "string" || relativePath.length === 0) return void 0;
	if (isAbsolute(relativePath) || /[\u0000-\u001f\u007f-\u009f]/u.test(relativePath) || !relativePath.toLowerCase().endsWith(".pdf")) return void 0;
	try {
		const storage = JSON.parse(await readFile(storagePath, "utf8"));
		const cwd = storage?.tables?.sessions?.[sessionId]?.identity?.cwd;
		...
		if (escaped === ".." || escaped.startsWith(`..${process.platform === "win32" ? "\\" : "/"}`) || isAbsolute(escaped)) return void 0;
		const data = await readFile(target);
		const { filename, bytes } = validatePdfBytes(data, target, maxBytes);
		return Object.freeze({ name: filename, mediaType: PDF_MEDIA_TYPE, data: bytes });
	} catch {
		return void 0;
	}
}

const MODEL_MODALITIES = ["text", "image", "pdf"];
...
function resolveModels(models, defaults) {
	...
	return (models ?? DEFAULT_MODELS).map((model) => {
		...
		if (inputModalities.some((modality) => !MODEL_MODALITIES.includes(modality))) throw new Error(`dsh-provider-veark: catalog model "${model.id}" inputModalities must contain only "text", "image" and "pdf"`);
...
function apply(ctx, config) {
	...
	const adapter = new VeArkAdapter({
		...
		resolveWorkspacePdf: (sessionId, path) => resolveWorkspacePdf(sessionId, path),
		mode
	});
