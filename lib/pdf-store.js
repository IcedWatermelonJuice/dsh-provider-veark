...
export function validatePdfBytes(data, name, maxBytes = DEFAULT_MAX_PDF_BYTES) {
...
export class VeArkPdfStore {
		...
		for (const [token, pair] of pairs) {
			if (pair.pdf !== void 0 && pair.json !== void 0) {
				try {
					...
					if (metadata.version !== 1 || metadata.token !== token || !Number.isFinite(createdAt)) continue;
					const stagedButUnused = metadata.usedAt === null;
					if (stagedButUnused ? now - createdAt < this.orphanGraceMs : retentionMs === 0 || now - createdAt < retentionMs) continue;
					await Promise.all([rm(pair.pdf, { force: true }), rm(pair.json, { force: true })]);
					if (stagedButUnused) orphans++;
					else expired++;
				} catch (error) {
	...
	async stage({ sessionId, name, mediaType, data }) {
		...
		const { filename, bytes } = decodePdf(data, name, this.maxBytes);
		try {
			await this.cleanup();
		} catch (error) {
	...
	async resolve(sessionId, token) {
		...
		const { filename } = validatePdfBytes(bytes, metadata.name, this.maxBytes);
		if (metadata.usedAt === null) try {
			await this.markUsed(metaPath, token);
		} catch (error) {
			this.log?.warn?.(`dsh-provider-veark: PDF usedAt update failed without blocking resolution: ${error?.message ?? error}`);
		}
		return Object.freeze({ name: filename, mediaType: PDF_MEDIA_TYPE, data: bytes });
	}

	async markUsed(metaPath, token) {
		await withFileLock(metaPath, async () => {
			const current = JSON.parse(await readFile(metaPath, "utf8"));
			if (current.version !== 1 || current.token !== token || current.usedAt !== null) return;
			current.usedAt = new Date(this.now()).toISOString();
			await writeFileAtomic(metaPath, `${JSON.stringify(current)}\n`, { mode: 0o600 });
		});
	}
}
