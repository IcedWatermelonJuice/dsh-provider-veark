...
function loadCard(openState) {
  ...
  const specs = {};
  for (const f of ["preferFiles","apiKeyEnv","chatBaseURL","filesBaseURL","filesApiKeyEnv","requestImagePixelBudget","requestImageMaxBytes","fileExpirySeconds","filesApiTimeoutMs","filesProbeIntervalMs"]) specs[f] = f === "preferFiles" ? { kind: "bool" } : { kind: "text" };
  const state = { available: true, writable: true, dirty: false, invalid: false, saving: false, failed: false, keyConfigured: false, keyWritable: true, keyRef: "ARK_API_KEY", keyValue: { text: "", overridden: false, invalid: false }, fields, models: [{ id: "ark-code-latest", name: "Ark Code Latest", contextWindow: 1000000, maxTokens: 128000, inputModalities: ["text", "image"], _contextRaw: "1M", _maxRaw: "128K", _contextInvalid: false, _maxInvalid: false }], modelsInvalid: false };
  ...
  const ctx = { card: null, remote: { $on: () => () => {} }, effect(fn) { fn(); }, get: (n) => n === "connection" ? { api: { credentials: { describe: async () => ({ result: { ok: true, value: { credentials: { ARK_API_KEY: { configured: false, writable: true } } } } }) } } } : undefined, locale: { bind: () => (k) => k, register() {} }, settingsScope: { bind: () => ({ getSnapshot: () => ({ status: "ready", writable: true, value: { preferFiles: true }, base: {}, user: {} }), subscribe: () => () => {}, set: async () => {}, unset: async () => {} }) }, slots: { inject: (name, thunk) => { for (const r of thunk()) ctx.card = r; }, register: (options, component) => ({ options, component }) } };
  exports.apply(ctx);
...
describe("真实 React 渲染", () => {
  test("收起态：仅 header，无表单", () => {
    ...
    const fields = { preferFiles: { checked: true, overridden: false, invalid: false }, apiKeyEnv: F, chatBaseURL: F, filesBaseURL: F, filesApiKeyEnv: F, requestImagePixelBudget: F, requestImageMaxBytes: F, fileExpirySeconds: F, filesApiTimeoutMs: F, filesProbeIntervalMs: F };
